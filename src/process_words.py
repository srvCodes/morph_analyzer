...
class ShiftWordsPerCW():
    def __init__(self, X, vocab_size, cw=4):
        self.window = cw
        self.X = X
    ...
    def shift_left(self, cw):
        ...
        while cw:
            ...
            x_left.append(' ')
            x_left.popleft()
            x_left_indexed = get_indexed_words(list(x_left), mode='use_vocab', vocab_size=self.vocab_size)
    ...
    def shift_right(self, cw):
        ...
        while cw:
            ...
            x_right.appendleft(' ')
            x_right.pop()
            x_right_indexed = get_indexed_words(list(x_right), mode='use_vocab', vocab_size=self.vocab_size)
...
def get_indexed_words(X, vocab_size, mode='build_vocab'):
    global x_char2idx
